...
from code.app.models.requests import RequestInput, RequestInfo, RequestUpdate
...
async def add(request_info: RequestInput, current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    return add_request(db, request_info, current_user.id)


@router.get("/all", response_model=List[RequestInfo])
async def get_all_requests(current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
...
async def get_cur_user_requests(current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    return get_user_requests(db, current_user.id)

@router.post("/update", response_model=None)
async def set_department(request_update: RequestUpdate, current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    if (get_user_roles(db, current_user.id) == Roles.ADMIN) or is_user_worker(db, current_user.id):
        return update_request_state(db, request_update)
    raise HTTPException(status_code=403, detail="Not enough permissions")


@router.get("/{request_id}", response_model=RequestInfo)
async def get_request_(request_id: str, current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    return get_request(db, request_id, current_user.id)

@router.get("/search/{phrase}", response_model=List[RequestInfo])
async def search_posts(phrase: str,current_user: Annotated[Users, Depends(get_current_active_user)], db: Session = Depends(get_db)):
