...
def edit_post_in_db(session: Session, post_id: str, post: PostBase):
    session.query(Posts).filter(Posts.id == post_id).update(
        {Posts.title: post.title, Posts.description: post.description, Posts.created_at: datetime.datetime.now()})
    session.commit()
...
def parse_post_to_response(session: Session, post: Posts) -> PostResponse:
    ...
    return PostResponse(id=post.id, title=post.title, description=post.description,
                        created_at=post.created_at, author_name=author_name, author_id=post.author_id)